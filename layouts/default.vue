  ...
  <div class="flex h-screen overflow-hidden bg-slate-50">
    ...
    <aside class="w-72 bg-slate-900 text-white flex flex-col transition-all duration-300 shadow-xl z-20">
      ...
      <nav class="flex-1 px-4 py-6 space-y-2 overflow-y-auto">
        ...
        <NavLink v-if="user?.role_name === 'ADMIN_PLANTEL' || user?.role_name === 'SUPER_ADMIN'" to="/reembolsos" icon="CurrencyDollarIcon">
          Mis Reembolsos
        ...
        <NavLink v-if="user?.role_name === 'REVISOR_OPS' || user?.role_name === 'SUPER_ADMIN'" to="/ops" icon="ClipboardDocumentCheckIcon">
          Revisión Operativa
        ...
        <NavLink v-if="user?.role_name === 'REVISOR_FISCAL' || user?.role_name === 'SUPER_ADMIN'" to="/fiscal" icon="ScaleIcon">
          Revisión Fiscal
        ...
        <NavLink v-if="user?.role_name === 'TESORERIA' || user?.role_name === 'SUPER_ADMIN'" to="/tesoreria" icon="BanknotesIcon">
          Tesorería
        ...
        <NavLink v-if="user?.role_name === 'RH' || user?.role_name === 'SUPER_ADMIN'" to="/rh" icon="FolderOpenIcon">Recursos Humanos</NavLink>
      ...
      <div class="p-6 border-t border-slate-800 bg-slate-900 space-y-4">
        ...
        <div v-if="user?.is_impersonating" class="rounded-xl border border-amber-400/30 bg-amber-500/10 px-3 py-2">
          <div class="text-[11px] uppercase tracking-wider text-amber-200 font-semibold">Impersonando</div>
          <div class="text-sm font-bold text-amber-100">{{ user.role_name }}</div>
          <button @click="stopImpersonation" class="mt-2 w-full text-xs font-semibold bg-amber-500/20 hover:bg-amber-500/30 text-amber-100 rounded-lg py-2 transition">Detener</button>
        </div>

        <div class="flex items-center gap-3">
          <div class="w-10 h-10 rounded-full bg-indigo-500 flex items-center justify-center text-sm font-bold text-white border-2 border-slate-700">
        ...
        <button @click="logout" class="w-full flex items-center justify-center gap-2 text-xs font-medium text-slate-400 hover:text-white hover:bg-slate-800 py-2 rounded transition-colors">
...
onMounted(() => {
  updateCounts();
  // Poll pending counts every 10 seconds to keep badges synced
